...
/* -------------------------------------------------------------
   ADMIN CREATE DRIVER WITH VALIDATION
------------------------------------------------------------- */
  ...
  try {
    ...
    if (!licenseRegex.test(licenseNumber)) {
      return res.status(400).json({
        success: false,
        message: "Invalid license number format. Expected XX00 00000000000"
      });
    }
    ...
    if (!aadharRegex.test(aadharNumber)) {
      return res.status(400).json({
        success: false,
        message: "Invalid Aadhaar number format. Must be 12 digits starting with 2-9"
      });
    }
    ...
    if (exists) {
      return res.status(400).json({ success: false, message: "Driver already exists (phone / license / aadhar)" });
    }
...
const loginDriver = async (req, res) => {
  try {
    ...
    const token = jwt.sign({ sub: driver._id, driverId: driver.driverId }, JWT_SECRET, {
      expiresIn: "1h"
    });
...
const updateFCMToken = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Failed to update FCM", error: err.message });
  }
};
